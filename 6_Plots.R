...
pdf(file = "Figures/VarImport3.pdf", width=9, height=5)
mat <- layout(rbind(c(1,5), c(2,5), c(3,6), c(4,6)), widths=c(3,3), heights=c(2,0.35,0.43,0.8), TRUE) 
#layout.show(mat)
...
plot(min(wl):max(wl), ylim=c(0,0.1), axes = FALSE, ylab="Median absolute deviation",
     xlab=expression(lambda(nm)), cex.lab = 1.3)
axis(1, pos=0, at = seq(0, 557, 100), labels = seq(400, 900, 100), las = 1, cex.axis = 1.3)
axis(2, pos=0, las=1, cex.axis = 1.3)
#lines(sens_N, col = "yellow")
lines(sens_all[1,1:557], col="black", lwd=2)
lines(sens_all[2,1:557], col="orange", lwd=2)
lines(sens_all[3,1:557], col="red", lwd=2)
lines(sens_all[4,1:557], col="green", lwd=2)
lines(sens_all[5,1:557], col="blue", lwd=2)
lines(sens_all[6,1:557], col="purple",lwd=2)
legend(5, 0.1, c("LAI", expression(C[ab]), expression(C[ar]), expression(C[m]), expression(C[w]), "ALA"), 
       col=c("black", "orange", "red", "green", "blue", "purple"), lwd=1, cex = 1.3,
       bty = "n", text.font = 3)
